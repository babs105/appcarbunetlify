...
class VehiculeListComponent extends Component {
  ...
  reloadVehiculeList = () => {
    ...
    vehiculeService.getAllVehicules().then((res) => {
  ...
  deleteVehicule = (immatricule) => {
    vehiculeService.deleteVehiculeByImmmatricule(immatricule).then((res) => {
      this.setState({ message: "Vehicule deleted successfully." });
      this.setState({
        cuves: this.state.vehicules.filter(
          (vehicule) => vehicule.immatricule !== immatricule
        ),
      });
    });
  };
                  ...
                  {this.state.loader ? (
                    ...
                    this.state.vehicules.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell
                          align="right"
                          onClick={() => this.editVehicule(row.immatricule)}
                        >
                          <CreateIcon />
                        </TableCell>
                        <TableCell
                          align="right"
                          onClick={() => this.deleteVehicule(row.immatricule)}
                        >
                          <DeleteIcon />
                        </TableCell>
                      </TableRow>
